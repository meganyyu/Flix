...
- (void)loadMovies {
    ...
    dispatch_async(dispatch_get_global_queue( QOS_CLASS_USER_INITIATED, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [hud hideAnimated:YES afterDelay:2.f]; // FIXME: MBProgressHUD only shows up if I manually set a delay time
...
- (void)fetchMovies {
    [[MovieAPIManager shared] fetchNowPlaying:^(NSArray *movies, NSError *error) {
        if (error != nil) {
